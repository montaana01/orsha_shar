...
export async function POST(request: NextRequest) {
  ...
  const rows = await query<{ file_name: string }>(
    'SELECT file_name FROM fonts WHERE id = ? AND is_deleted = 1 LIMIT 1',
    [id],
  );
  const font = rows[0];
  if (!font) {
  ...
  const restored = await restorePath(publicDir, path.join('fonts', font.file_name));
  if (!restored) {
    const url = buildRedirectUrl(request, '/yakauleu');
    if (tab) url.searchParams.set('tab', tab);
    if (section) url.searchParams.set('section', section);
    url.searchParams.set('error', 'fonts-missing');
    return NextResponse.redirect(url, 303);
  }

  await execute('UPDATE fonts SET is_deleted = 0, visible = 1 WHERE id = ?', [id]);
