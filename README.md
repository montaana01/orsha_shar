...
Современная замена старого билда на Tilda: быстрый SEO-дружественный сайт-витрина для услуг оформления шарами (Орша) с галереями по категориям и “конструктором заявки” без хранения данных.
...
- **Конструктор заявки** `/constructor`:
...
- `src/app/`
  ...
  - `/constructor/page.tsx` — конструктор заявки (клиентский UI)
...
- Конструктор `/constructor` генерирует текст и помогает передать его в мессенджер, без отправки данных на сервер.
